...
accy_fcst_pivot <- reactive({
  filter(mega_data_by_r, 
         product_platform_name == input$product_platform,
...
```{r}
reshape2::dcast(mega_data_by_r, lag ~ forecast_per, value.var = "actual", sum) %>%
    dplyr::mutate(values = "Actual Cases") %>% 
    dplyr::relocate(lag, values)
...
mega_data_by_r %>% 
  ...
  dplyr::ungroup() %>% 
  data.frame() %>% 
  tidyr::pivot_wider(names_from = forecast_per, values_from = actual) -> a

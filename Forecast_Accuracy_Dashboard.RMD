...
forecast_per <- unique(sort(mega_data_by_r$forecast_per))
pickerInput(inputId = "forecast_per", label = "Forecast Per", choices = forecast_per,
                    options = list("actions-box" = TRUE), multiple = TRUE, width = 500, selected = forecast_per)
...
renderDataTable({
  ...
  accy_fcst_pivot_2 <- reshape2::dcast(accy_fcst_pivot, lag ~ forecast_per, value.var = "actual", sum)
